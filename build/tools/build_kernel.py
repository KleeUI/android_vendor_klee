...
def validate_exclusive_kernel_outputs(args, dist):
    """Ensure staging did not resurrect a second provider from the platform."""
    selected = selected_kernel_module_names(args)
    installed = {
        path.name
        for path in dist.rglob("*.ko")
        if path.is_file()
    }
    for group_name, candidates in KLEE_EXCLUSIVE_KERNEL_MODULE_GROUPS.items():
...
def configure_external_spec_sync_provider(args, platform, platform_env):
    """Disable the legacy in-tree provider when external sync_fence is used.

    The Qualcomm platform Kconfig defaults QCOM_SPEC_SYNC to m while the
    external mm-drivers tree uses the same symbol name for its own Kbuild.
    A post-defconfig fragment keeps those namespaces independent: the kernel
    tree is built without its legacy character device, while the external
    provider still receives its exported configuration header.
    """
    if "sync_fence.ko" not in selected_kernel_module_names(args):
        return

    fragment = args.out / ".klee-disable-in-tree-spec-sync.config"
    fragment.write_text(
        "# Generated by Klee; keep the external sync_fence provider unique.\n"
        "klee_disable_in_tree_spec_sync() {\n"
        "  \"$KERNEL_DIR/scripts/config\" --file \"$OUT_DIR/.config\" -d QCOM_SPEC_SYNC\n"
        "}\n"
        "append_cmd POST_DEFCONFIG_CMDS 'klee_disable_in_tree_spec_sync'\n",
        encoding="utf-8",
    )
    relative = pathlib.PurePosixPath(os.path.relpath(fragment, platform)).as_posix()
