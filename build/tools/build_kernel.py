...
KLEE_EXCLUSIVE_KERNEL_MODULE_GROUPS = {
    # Both modules claim the same /dev/spec_sync character device.  The
    # external Qualcomm implementation is the provider for the display
    # wait/bind ABI; allowing the in-tree legacy provider beside it makes
    # module load order nondeterministic and can strand the display driver.
    "speculative sync provider": frozenset(
...
def selected_kernel_module_names(args):
    """Return the module basenames selected by the bundle declaration."""
    names = set(args.required_module)
    names.update(
        pathlib.PurePosixPath(value).name + ".ko"
        for value in args.external_module
    )
    return names


def validate_exclusive_kernel_modules(args):
    """Reject a bundle declaration that selects two providers for one ABI."""
    selected = selected_kernel_module_names(args)
    for group_name, candidates in KLEE_EXCLUSIVE_KERNEL_MODULE_GROUPS.items():
        present = sorted(selected.intersection(candidates))
        if len(present) > 1:
            raise ValueError(
                f"{group_name} has multiple providers: {', '.join(present)}"
            )
            ...
            for profile in ("qca6490", "qca6750"):
                ...
                extras = [
                    ...
                    *KLEE_WLAN_PROFILE_PLATFORM_CONFIG,
                    ...
                    # The standalone profile can miss both ce_api.h and
                    # multibus.h: the shared Kbuild only adds the CE and
                    # dispatcher directories for a CONFIG_HIF_* branch,
                    # which is not selected while Klee parses the external
                    # profile.  Keep the paired host common headers visible
                    # through command-line flags without modifying the
                    # imported Qualcomm tree.
                    f"KCFLAGS=-I{module / 'cmn' / 'hif' / 'src' / 'ce'} -I{module / 'cmn' / 'hif' / 'src' / 'dispatcher'}",
...
def validate_exclusive_kernel_outputs(args, dist):
    """Ensure no output tree resurrects a second exclusive provider.

    Qualcomm's mixed build keeps the platform output below ``args.out`` while
    the final KERNEL_KIT is copied to ``dist``.  Checking only the latter can
    miss a legacy module left in the platform staging tree, which would later
    be picked up by vendor_dlkm assembly.
    """
    selected = selected_kernel_module_names(args)
    roots = [dist]
    if args.platform_root:
        roots.append(args.out / args.source.relative_to(args.platform_root))
    installed = set()
    for root in roots:
        if root.is_dir():
            installed.update(
                path.name for path in root.rglob("*.ko") if path.is_file()
            )
    for group_name, candidates in KLEE_EXCLUSIVE_KERNEL_MODULE_GROUPS.items():
        present = sorted(installed.intersection(candidates))
        if len(present) > 1:
            raise RuntimeError(
                f"{group_name} produced multiple modules: {', '.join(present)}"
            )
        if len(present) == 1 and present[0] not in selected:
            raise RuntimeError(
                f"{group_name} produced unselected provider {present[0]}"
            )


def configure_cupid_source_module_configs(args, platform, platform_env):
    """Enable generic kernel modules named by Cupid's source manifest.

    The public Waipio defconfig intentionally leaves optional MTD, IR and
    accessory drivers unset.  Cupid's DTS uses the MTD diagnostic path and
    IR-SPI, while its source-owned module contract also keeps the generic USB
    Ethernet and FAN53555 drivers available.  Apply these symbols only to the
    outer msm-kernel output; the nested GKI output has a different OUT_DIR and
    must retain its upstream config.
    """
    expected_out = args.out / args.source.relative_to(platform)
    helper = args.out / ".klee-enable-cupid-source-modules.sh"
    symbols = (
        "MTD",
        "MTD_BLOCK",
        "MTD_BLOCK2MTD",
    ...
    helper.write_text(
        "#!/bin/bash\n"
        "set -euo pipefail\n"
        f"expected_out={shlex.quote(str(expected_out))}\n"
        f"kernel_source={shlex.quote(str(args.source))}\n"
        "current_out=\"$(readlink -m \"${OUT_DIR:-/nonexistent}\")\"\n"
        "if [ \"$current_out\" != \"$expected_out\" ]; then exit 0; fi\n"
        f"\"$kernel_source/scripts/config\" --file \"$expected_out/.config\" {config_args}\n"
        "make -C \"$kernel_source\" O=\"$expected_out\" olddefconfig\n",
        encoding="utf-8",
    )
    helper.chmod(0o755)
    fragment = args.out / ".klee-cupid-source-modules.config"
    relative_helper = pathlib.PurePosixPath(
        os.path.relpath(helper, platform)
    ).as_posix()
    fragment.write_text(
        "# Generated by Klee; enable Cupid generic source modules in outer msm-kernel.\n"
        ...
        for profile in ("qca6490", "qca6750"):
            ...
            command = [
                ...
                *KLEE_WLAN_PROFILE_PLATFORM_CONFIG,
                ...
                # CE and dispatcher headers are required even when no
                # CONFIG_HIF_* branch is active during Kbuild parsing.
                f"KCFLAGS=-I{qcacld / 'cmn' / 'hif' / 'src' / 'ce'} -I{qcacld / 'cmn' / 'hif' / 'src' / 'dispatcher'}",
...
def build_kernel_platform(args, top, make, jobs, env, layout, layout_digest):
    ...
    # External and platform installers may place modules below nested
    # lib/modules or extra directories.  Remove every old provider before the
    # transaction so the exclusivity check cannot be tripped by stale output
    # and, more importantly, stale modules cannot leak into vendor_dlkm.
    stale_outputs.extend(args.dist.rglob("*.ko"))
    ...
    if args.external_module:
        ...
        if external_script is not None:
            ...
            platform_env["DIST_CMDS"] = " && ".join(dist_phases)
    configure_cupid_source_module_configs(args, platform, platform_env)
    configure_external_spec_sync_provider(args, platform, platform_env)
    if args.dt_layout:
    ...
    validate_exclusive_kernel_outputs(args, args.dist)
